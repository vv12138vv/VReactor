...
public:
  explicit MemoryPool(size_t page_size = MP_PAGE_SIZE,
                      size_t align = MP_ALIGNMENT)
      : page_size_(page_size), align_(align) {
  ...
  static void *align_ptr(void *p, size_t align) {
    return reinterpret_cast<void *>((((size_t)p + (align - 1)) & ~(align - 1)));
  }
